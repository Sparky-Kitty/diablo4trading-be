...
const BID_ERROR_MESSAGES = {
    ...
    INVALID_BID_OPENING: (openingBid: number, bidIncrement: number) =>
        `Bid amount is less than the listing's opening bid (${openingBid}) + minimum bid increment (${bidIncrement}).`,
    INVALID_BID_BUY: "Buyout amount must be greater than the listing's buy now price.",
    INVALID_BID_CURRENT: (currentBidPrice: number, bidIncrement: number) =>
        `Bid amount must be greater than the current bid price (${currentBidPrice}) + minimum bid increment (${bidIncrement}).`,
        ...
        return await manager.transaction(async (transactionalManager: EntityManager) => {
            ...
            if (!itemListing) {
                throw new ServiceResponseException(
                    BID_ERROR_CODES.LISTING_NOT_FOUND,
                    BID_ERROR_MESSAGES.LISTING_NOT_FOUND,
                );
            }
            ...
            if (itemListing.state !== ItemListingState.ACTIVE) {
                throw new ServiceResponseException(
                    BID_ERROR_CODES.LISTING_INACTIVE,
                    BID_ERROR_MESSAGES.LISTING_INACTIVE,
                );
            }
            ...
            if (type === 'buyout' && buyNowPrice) {
                if (bidAmount < buyNowPrice) {
                    throw new ServiceResponseException(
                        BID_ERROR_CODES.INVALID_BID_BUY,
                        BID_ERROR_MESSAGES.INVALID_BID_BUY,
                    );
                }
            ...
            if (!validBid && currentBidPrice) {
                if (bidAmount <= currentBidPrice + bidIncrement) {
                    throw new ServiceResponseException(
                        BID_ERROR_CODES.INVALID_BID_CURRENT,
                        BID_ERROR_MESSAGES.INVALID_BID_CURRENT(currentBidPrice, bidIncrement),
                    );
                }
            } else if (!validBid && bidAmount < openingBid + bidIncrement) {
                throw new ServiceResponseException(
                    BID_ERROR_CODES.INVALID_BID_OPENING,
                    BID_ERROR_MESSAGES.INVALID_BID_OPENING(openingBid, bidIncrement),
                );
            }
